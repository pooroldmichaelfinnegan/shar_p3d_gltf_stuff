...
with open('./l1z1_intersectdsg.json', 'rt') as p3djson_file:
...
with open('./n3_pygltf_intersect.bin', 'w+b') as binary:
    print(len(pos_buffer), len(ind_buffer))
...
# _glTF["buffers"] += [buf()]
_glTF["buffers"][0]["uri"] = bin_uri
_glTF["buffers"][0]["byteLength"] = len(pos_buffer+ind_buffer)
_glTF["meshes"] += [msh()]

with open('./n3_pygltf_intersect.gltf', 'wt') as outgltf:
    # outgltf.write(gltf)
